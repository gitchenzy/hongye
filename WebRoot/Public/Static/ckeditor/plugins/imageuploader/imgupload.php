...
if (!isset($_SESSION['WEMALL_ADMIN_']['CurrentUser'])) {
    die("未登录");
}
$currentUser = $_SESSION['WEMALL_ADMIN_']['CurrentUser'];
...
$imgnumber = count(scandir(__ROOT__ . $target_dir));
$filename = "$imgnumber$randomLetters.$ext";
$target_file = __ROOT__ . $target_dir . $filename;
$ckfile = $useruploadpath . $filename;
$uploadOk = 1;
...
if($check !== false) {
    ...
    echo "<script>alert('上传文件不是图片.');</script>";
    $uploadOk = 0;
}
...
if ($_FILES["upload"]["size"] > 512000) {
    echo "<script>alert('上传文件最大支持512KB.');</script>";
    $uploadOk = 0;
}
// Allow certain file formats
if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
&& $imageFileType != "gif" && $imageFileType != "ico" ) {
    echo "<script>alert('对不起，只能上传JPG, JPEG, PNG & GIF 文件.');</script>";
    $uploadOk = 0;
}
